...
def get_ml_gs():
    qs = Event.objects.raw('''select t.game_id,
        ...
        from (select e.game_id, e.away, e.home,
        ...
        as h_sprd_val
        from sportsbook_event as e
        left join betslip_betvalue as bv on e.game_id = bv.event_id
        where e.live_status = 0 and bv."type" = 'h_sprd' or bv."type"='a_sprd'
        group by e.game_id
        ) as t
...
def get_sprd_qs():
    qs = Event.objects.raw('''select t.game_id,
        ...
        from (select e.game_id, e.away, e.home, MAX(bv.handicap) as handicap,
        ...
        from sportsbook_event as e
        left join betslip_betvalue as bv on e.game_id = bv.event_id
        where e.live_status = 0 and bv."type" = 'h_line' or bv."type"='a_line'
        group by e.game_id
        ) as t
        order by val desc limit 10''')
    return qs
